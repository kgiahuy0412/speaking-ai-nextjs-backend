...
export async function prepareWorkerConversation(input: {
  ...
  const operation = (async () => {
    ...
    // Publish the playable URL immediately. Durable persistence continues in
    // the route's after() work so Safari can start buffering without waiting
    // for another Neon round trip on the critical path.
